            ...
            <li v-for="(menuItem, index) in menuItems" :key="index" class="sidebar-list"
                ...
                <a href="javascript:void(0)" class="sidebar-link sidebar-title" :class="{ 'active': menuItem.active }"
                    ...
                    <svg class="stroke-icon">
                        <use :xlink:href="(iconSprite.default || iconSprite) + `#${menuItem.icon}`"></use>
                    </svg>
                    <svg class="fill-icon">
                        <use :xlink:href="(iconSprite.default || iconSprite) + `#${menuItem.iconf}`"></use>
                    </svg>
                    <span class="lan-3">
                        {{ menuItem.title }}
                    </span>
...
export default {
    ...
    data() {
        return {
            ...
            text: '',
            active: false,
            // Perbaikan untuk Webpack 5: Pastikan kita mengambil string URL dari objek modul
            iconSprite: (iconSpritePath && iconSpritePath.default) ? iconSpritePath.default : iconSpritePath,
        };
    },
    computed: {
        ...mapState({
    ...
    created() {
        ...
        setTimeout(() => {
            ...
            if (elmnt) {
                this.$store.state.menu.menuWidth = elmnt.offsetWidth || 0;
